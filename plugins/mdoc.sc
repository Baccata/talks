...
/**
  * Based on MDoc module from https://github.com/satabin/swam
...
trait MdocModule extends ScalaModule {
  ...
  /**
    ...
    */
  def mdocClasspath: T[Agg[PathRef]] =
    T {
  ...
  def mdoc: T[PathRef] =
    T {
      val inputs = mdocSourceDirectory().map(_.path.toIO.getAbsolutePath)
      val target = T.ctx().dest
      inputs.foreach { in =>
        val out = target.toIO.getAbsolutePath
        val re = mdocNameFilter()
        val site = mdocSite().flatMap { case (k, v) => Seq(s"--site.$k", v) }
        val opts = mdocScalacOptions()
        val pOpts = mdocPluginJars().map(pathRef =>
        ...
        val mdocArgs = List(
          ...
          (opts ++ pOpts).mkString(" "),
          "--no-link-hygiene"
        ) ++ re.toSeq.flatMap(Seq("--include", _)) ++ site
        val mdocClassPath = mdocClasspath().map(_.path)
